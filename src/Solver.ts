...
export abstract class SolverBase{
    ...
    abstract solve(startPt: Point, endPt: Point, adjMat : number[][],points : Point[]) : Point[];
}
...
export class DijkstraSolver extends SolverBase{
    ...
    solve(startPt: Point, endPt: Point, adjMat : number[][],points : Point[]): Point[] {
        ...
        if(curIdx !== -1){
            while(parents[curIdx] !== -1){
                path.push(points[parents[curIdx]]);
